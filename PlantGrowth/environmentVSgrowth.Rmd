...
# worldclim data
worldclim <- read_delim('/Volumes/nordborg/user/pieter.clauw/Documents/Source/Accessions/List_2029_latlon_env.txt', delim = '\t')
...
growthClim <- inner_join(growth.long, clim, by = c('accession' = 'id')) %>%
...
for (pheno in phenotypes)
{
  ...
  for (temp in c(temperatures, 'response'))
  {
    ...
    x <- growthClim %>%
      ...
      select(pheno, everything(), -accession, -nonPheno, -temperature, -lng, -lat, -name, -country, -CS_number, -group) %>%
      select_if(~ !any(is.na(.))) %>%
...
# Train the model
    step.model <- train(LGR ~., data = growthClim_WC,
                    method = "leapSeq", 
