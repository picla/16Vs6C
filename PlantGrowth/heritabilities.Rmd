...
```{r raw data H}
h.raw <- data.frame('DAS' = rep(DASs, length(temperatures)), 'temperature' =  rep(temperatures, each = length(DASs)), 'H' = NA, stringsAsFactors = F)
h.predict <- data.frame('DAS' = rep(DASs, length(temperatures)), 'temperature' = rep(temperatures, each = length(DASs)), 'H' = NA, stringsAsFactors = F)
for (t in temperatures)
{
  for (DAS in DASs)
  {
    lemna.d.t <- lemna[lemna$DAS == DAS & lemna$temperature == t, c('acn', 'logArea')]
    predictData.d.t <- predictData[predictData$DAS == DAS & predictData$temperature == t, c('acn', 'logArea')]
    h.raw$H[h.raw$DAS == DAS & h.raw$temperature == t] <- H(lemna.d.t, print = F)[4]
    h.predict$H[h.predict$DAS == DAS & h.predict$temperature == t] <- H(predictData.d.t, print = F)[4]
  }
}
```
