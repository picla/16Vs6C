...
# get plasma colours fro temperature on continuous scale
...
#TEST#
plot(autumnTemp$meanAutumn, autumnTemp$latitude, col = autumnTemp$autumnTempCol)
gradientLegend(valRange = autumnTemp.legend$autumnTemp, color = autumnTemp.legend$autumnTempCol, pos = 0.225, side = 3, inside = T, pos.num = 1)
```
...
pdf('/Volumes/nordborg/user/pieter.clauw/Documents/Experiments/UltimateQandD/Results/Growth/Plots/logArea_overtime_tempclustCol_16C_6C.pdf', height = 5, width = 14)
plot(NA, NA, xlim = c(minDAS, maxDAS), ylim = c(minArea, maxArea), xlab = 'DAS (days after stratification)', ylab = 'log(rosette area)')
for (t in temperatures)
{
...
GWAS.Y$autumnTemp <- autumnTemp$meanAutumn[match(GWAS.Y$accession, autumnTemp$acn)]
