...
fm.nlme.1 <- nlme(fm1.power.lis, 
                       control = ctrl)

# get starting values
fm.nlme.1.FE <- fixef(fm1.power.nlme)
n.acn <- length(unique(lemna.sub.grp.DAS$acn)) - 1
n.temp <- length(unique(lemna.sub.grp.DAS$temperature)) - 1
n <- n.acn + n.temp + n.acn * n.temp
fm.start <- c(fm1.power.nlme.FE[1], rep(0, n), fm1.power.nlme.FE[2], rep(0, n), fm1.power.nlme.FE[3],rep(0, n))

fm.nlme.2 <- update(fm1.power.nlme,
                         fixed = list(a + b + c ~ acn * temperature),
                         start = fm.start,
                         weights = varExp(form = ~DAS),
                         control = ctrl)
