...
{
  fit.lis <- nlsList(Area ~ SS.pow(DAS_decimal, M0, r, beta) | acn,
                         data = data,
                         control = ctrl.nls)
  fit.nlme <- nlme(fit.lis,
              control = ctrl.nlme,
...
{
  fit.lis <- nlsList(Area ~ SSlogis(DAS_decimal, Asym, xmid, scal) | acn,
                         data = data,
                         control = ctrl.nls)
  fit.nlme <- nlme(fit.lis,
              control = ctrl.nlme,
...
# 2 convergence - 16C rep2 (104 iterations)
...
# 3 convergence - 16C rep3 (16 iterations)
...
fit.nlme.poweco.6C <- model.nlme.poweco(lemna.sub.6C)
```
