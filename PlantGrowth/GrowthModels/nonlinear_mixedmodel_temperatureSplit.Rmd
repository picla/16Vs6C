...
accessions <- unique(lemna.6C$acn)
predData <- data.frame(
  acn = rep(accessions, each = length(DAS_decimal_pred)),
  DAS_decimal= rep(DAS_decimal_pred, length(accessions)))

predData$fit5 <- predict(fit5.nlme.6C, newdata = predData, level = 0)
predData$fit4 <- predict(fit4.nlme.6C, newdata = predData, level = 0)
predData$fit3 <- predict(fit3.nlme.6C, newdata = predData, level = 0)

pdf('/groups/nordborg/user/pieter.clauw/Documents/Experiments/UltimateQandD/Results/Growth/nonlinear/augPred_fit3_6C.pdf')
for(acn in accessions)
{
  ...
  print(ggplot(predData.a, aes(DAS_decimal, fit3)) +
    geom_line() +
    geom_point(data = lemna.6C.a, aes(DAS_decimal, Area, group = replicate)))
}
dev.off()
...
# CONVERGES
fit2a.nlme.6C <- nlme(Area ~ SS.pow(DAS_decimal, M0, r, beta),
                    ...
                    control = ctrl.nlme,
                    verbose = T)
save(fit2a.nlme.6C, file = '/groups/nordborg/user/pieter.clauw/Documents/Experiments/UltimateQandD/Results/Growth/nonlinear/fit2a.nlme.6C.rda')
# CONVERGES
fit2b.nlme.6C <- nlme(Area ~ SS.pow(DAS_decimal, M0, r, beta),
                    ...
                    control = ctrl.nlme,
                    verbose = T)
save(fit2b.nlme.6C, file = '/groups/nordborg/user/pieter.clauw/Documents/Experiments/UltimateQandD/Results/Growth/nonlinear/fit2b.nlme.6C.rda')
...
fit2c.nlme.6C <- nlme(Area ~ SS.pow(DAS_decimal, M0, r, beta),
                    ...
                    control = ctrl.nlme,
                    verbose = T)
save(fit2c.nlme.6C, file ='/groups/nordborg/user/pieter.clauw/Documents/Experiments/UltimateQandD/Results/Growth/nonlinear/fit2c.nlme.6C.rda')
