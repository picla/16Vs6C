...
lemnaSlim.poly <- lme(logArea ~ (DAS + I(DAS^2) + I(DAS^3)) * acn * experiment, random = ~ 1 + DAS|experiment/ID, data = lemnaSlim, na.action = na.exclude, control=lmeControl(singular.ok=TRUE, returnObject=TRUE), correlation = corCAR1(form = ~ DAS|experiment/ID))

#save(lemnaSlim.poly, file = "Results/Growth/poly3_logRosette.rda")
save(lemnaSlim.poly, file = "Results/Growth/poly3_logRosette_NO_allReps_V3_corCAR1_expFixed_interaction.rda")

# residuals
pdf('Results/Growth/Plots/poly3_logRosette_NO_allReps_resiudals_V3_corCAR1_expFixed_interaction.pdf')
plot(lemnaSlim.poly)
dev.off()
