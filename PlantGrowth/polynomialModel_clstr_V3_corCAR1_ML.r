...
# using maximum likelhood instead of REML. sole use to compare between model. Final estimnates shoudl be taken from REML mnodels.
...
lemnaSlim.poly <- lme(logArea ~ (DAS + I(DAS^2) + I(DAS^3)) * acn * temperature, random = ~ 1 + DAS|experiment/ID, data = lemnaSlim, na.action = na.exclude, control=lmeControl(singular.ok=TRUE, returnObject=TRUE), correlation = corCAR1(form = ~ DAS|experiment/ID), method = 'ML')

#save(lemnaSlim.poly, file = "Results/Growth/poly3_logRosette.rda")
save(lemnaSlim.poly, file = "Results/Growth/poly3_logRosette_NO_allReps_V3_corCAR1_ML.rda")

# residuals
pdf('Results/Growth/Plots/poly3_logRosette_NO_allReps_resiudals_V3_corCAR1_ML.pdf')
plot(lemnaSlim.poly)
dev.off()
...
write.table(emm, file = "Results/Growth/lemna_DAS_allReps_NO_logRosette_emmeans_V3_CorCAR1_ML.txt", sep = "\t", quote = F, row.names = F)
