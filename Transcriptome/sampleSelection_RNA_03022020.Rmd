---
title: "sampleSelection_RNA_03022020"
...
# samples to select
samples <- tibble(
  accession = c(6017, 9559, rep(c(8242, 9075, 9433, 9888), each = 6)),
  temperature = c(rep('6C', 2), rep(rep(temperatures, each = 3), 4)),
  replicate = c(rep('rep1', 2), rep(replicates, 8))
)
```
...
for (i in 1:nrow(samples))
{
  ...
  boxName <- paste('boxes_', samples$temperature[i], '_', samples$replicate[i], '.txt', sep = '')
...
selection <- selection[order(selection$Box, selection$Experiment), ]
write.table(selection, file = 'Setup/Metabolite_RNA_selection/RNA_7_selection.txt', quote = F, row.names = F, sep = '\t')
```
